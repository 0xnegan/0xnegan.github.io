---
icon: fas fa-crosshairs
order: 4
title: Arsenal
---

## Public Frameworks

### Network Penetration Testing — Complete Methodology
200+ saves · 7,000+ visits · Community-adopted reference. Comprehensive network pentesting framework — reconnaissance through post-exploitation with full command reference.

[**View on Notion →**](https://spangle-snail-0fe.notion.site/Network-Penetration-Testing-Complete-Methodology-48b89f2cef9d41c1a7988acdd8c8a0fb)

### Red Team Operations & Adversary Simulation Framework
End-to-end adversary simulation methodology: infrastructure, initial access, execution, persistence, lateral movement, exfiltration. Maps to MITRE ATT&CK with real-world CRTO tradecraft.

[**View on Notion →**](https://chemical-azimuth-aa6.notion.site/Red-Team-Operations-Adversary-Simulation-2e45fe61924981098a08c710b06fa8c1)

---

## 🔒 Private Repositories

### 🔒 Web Application Penetration Testing — Complete Methodology
> *Restricted access — shared with select community members*

Comprehensive, structured methodology for web application penetration testing covering all phases from reconnaissance to post-exploitation. Each section contains practical techniques, tools, and real-world attack vectors organized for operational use.

### 🔒 Get-Shell Specialist — Complete Arsenal
> *Restricted access — shared with select community members*

Single source of truth for obtaining shells on web applications and servers. Every technique, bypass, payload, and methodology compiled from top researchers, real-world engagements, and cutting-edge research. Designed for consistent shell acquisition regardless of target hardening.

Includes **Wingman** — operational helper directory that guides through the full attack path from initial IP address to final reporting. *START HERE: Your Operational Wingman ^.^*

---

## Projects

### CipherSight AI Security Framework

Automated pentesting framework integrating:

1. **Reconnaissance** — Multi-tool recon → fingerprinting → alive subdomain filtering → endpoint and directory discovery
2. **Detection** — Nuclei-powered vulnerability scanning
3. **AI Classification** — Fine-tuned RoBERTa model trained on NVD CVE data (2016-2023) for accurate vulnerability classification
4. **AI Reporting** — Executive summary generation with automated mitigation engine providing context-aware remediation strategies

---

## Research — Published & Upcoming

| Topic | Status | Category |
|---|---|---|
| Living Off the User — Post-Credential Phishing | **Published** | Red Team / Phishing |
| The OSINT Kill Chain | **Published** | OSINT / Intelligence |
| AI Architecture Attacks — Data Poisoning to Privacy | Coming Soon | AI Red Teaming |
| Multi-Cloud Red Teaming | Planned | Cloud Security |

---

## Channels

| Platform | Link |
|---|---|
| **YouTube** | [@negansec](https://youtube.com/@negansec) |
| **Medium** | [@CipherHawk](https://medium.com/@CipherHawk/) |
| **LinkedIn** | [Profile](https://www.linkedin.com/in/ahmed-13b6bb279) |
| **Telegram** | Macroc |
| **Discord** | #PCProdigy5311 |
